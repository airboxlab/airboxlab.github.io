...
We then demonstrate it can overperform policies with "classic" models on a series of toy tasks (2D grid path-finding) and simulated HVAC environments.
...
<center class="image-foot"><i>fig. 2: 2 agents taking independent decisions on the same state, leading to a credit assignment problem</i></center>
<br/>
...
The first task is a toy problem where an agent must navigate through a 2D-grid and maximize its reward. The action space is designed with 2 actions (vertical and horizontal movements). 
To emphasize on the need to learn correlation between actions, the reward scheme is modeled using 2 gaussian distributions located on 2 corners, and a one-time bonus whenever the agent reaches a point close to the mean of each gaussian. We refer to this training environment as bi-modal environment (BME). The agent start each episode starting at the center of the grid. Episode ends when all bonuses are found.
...
We run 5 experiments with different seeds for each method (classic and AR) and compare the mean return per episode below.
